...
\begin{enumerate}
\item \textbf{Native NaN handling.} Broker scores from Fink are ${\sim}100\%$ NaN for most ZTF objects, and forced-photometry ALeRCE classifiers cover only ${\sim}20\%$. RandomForest required median imputation, which is lossy. LightGBM learns the optimal split direction for missing values during tree construction, meaning the \emph{pattern of missingness itself} becomes an informative signal.
...
As of April 2026, the repository already communicates that idea pretty well and implements both the data pipeline and the full training layer. All classifiers have been upgraded from RandomForest to LightGBM with native NaN handling, proper regularisation, and calibration. The remaining work is mostly external assets (expert weights, stronger truth labels), not missing in-repo code.
