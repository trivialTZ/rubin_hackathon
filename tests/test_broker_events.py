...
def test_broker_events_preserve_fink_temporal_keys(tmp_path: Path) -> None:
    ...
    assert silver_df.loc[0, "expert_key"] == "fink/snn"
    assert silver_df.loc[0, "n_det"] == 2
    assert silver_df.loc[0, "event_time_jd"] == 2460001.5
    assert silver_df.loc[0, "temporal_exactness"] == "exact_alert"
