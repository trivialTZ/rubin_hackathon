...
3. Add to `DEFAULT_FEATURES` in `src/debass_meta_meta/models/early_meta.py` only if the baseline should also use it

If you add a new expert/broker field:
1. Preserve it in `data/silver/broker_events.parquet`
2. Update the relevant projector in `src/debass_meta_meta/projectors/`
